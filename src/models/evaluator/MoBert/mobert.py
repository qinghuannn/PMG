...
class TMEvalTokenizer:
    ...
    def tokenize(
        ...
        text_ids = torch.cat([text_ids,
                              torch.zeros(text_ids.shape[0], max_token_len - text_ids.shape[1], dtype=torch.long,
                                          device=device)], dim=1)
        text_mask = torch.cat([text_mask,
                               torch.zeros(text_mask.shape[0], max_token_len - text_mask.shape[1], dtype=torch.long,
                                           device=device)], dim=1)
        ...
        text_start_token = text_embedder(torch.tensor([self.convert_tokens_to_ids("[SOT]")], device=device)).repeat(
            sample_len, 1, 1)
        ...
        cls_emb = text_embedder(torch.tensor([self.convert_tokens_to_ids("[CLS]")], device=device))[None, ...].repeat(
            sample_len, 1, 1)
        ...
        if (motion_embs is not None):
            ...
            start_token = text_embedder(torch.tensor([self.convert_tokens_to_ids("[MOT]")], device=device)).repeat(
                sample_len, 1, 1)
        ...
        final_embs = final_embs + position_embedder(torch.arange(0, max_context_len, device=device))[None, ...].repeat(
            sample_len, 1, 1)
...
class MotionTextEvalBERT(nn.Module):
    def __init__(self, primary_evaluator_model_config, chunk_encoder_config, tokenizer_and_embedders_config,
                 tokenizer_path="", load_trained_regressors_path=""):
        super().__init__()
        ...
        if (self.load_trained_regressors_path):
            self.naturalness_regressor = load_pickle(
                Path(self.load_trained_regressors_path) / "SVR_PROB_CLS_MOTIONTEXT_Naturalness.obj")
            self.faithfulness_regressor = load_pickle(
                Path(self.load_trained_regressors_path) / "SVR_PROB_CLS_MOTIONTEXT_Faithfulness.obj")
        ...
        self.chunk_encoder = nn.Sequential(
            nn.Linear(chunk_encoder_config["frame_enc_dim"] * (
                        chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"]),
                      chunk_encoder_config["frame_enc_dim"] * (
                                  chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"])),
            nn.GroupNorm((chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"]),
                         chunk_encoder_config["frame_enc_dim"] * (
                                     chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"])),
            nn.SELU(),
            nn.Dropout(float(chunk_encoder_config["dropout"])),
            nn.Linear(chunk_encoder_config["frame_enc_dim"] * (
                        chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"]),
                      chunk_encoder_config["frame_enc_red_dim"] * (
                                  chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"])),
            nn.GroupNorm(chunk_encoder_config["frame_enc_groups"], chunk_encoder_config["frame_enc_red_dim"] * (
                        chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"])),
            nn.SELU(),
            nn.Dropout(float(chunk_encoder_config["dropout"])),
            nn.Linear(chunk_encoder_config["frame_enc_red_dim"] * (
                        chunk_encoder_config["chunk_size"] + chunk_encoder_config["chunk_overlap"]),
                      chunk_encoder_config["enc_dim"]),
        ...
        self.tokenizer = TMEvalTokenizer(self.tokenizer_train_method, self.tokenizer_train_path,
                                         vocab_size=self.vocab_size, saved_tokenizer_path=tokenizer_path,
                                         use_seg_embs=self.use_seg_embs)
        self.word_embs, self.pos_embs, self.seq_embs = self.tokenizer.get_initial_embeddings(
            primary_evaluator_model_config["enc_dim"], tokenizer_and_embedders_config["max_context_length"])
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.enc_dim, nhead=self.num_heads,
                                                   dropout=float(primary_evaluator_model_config["dropout"]),
                                                   dim_feedforward=primary_evaluator_model_config["dim_ff"],
                                                   batch_first=True)
        ...
        self.token_pred_head = nn.Sequential(
            ...
            nn.SELU(),
            nn.Dropout(float(chunk_encoder_config["dropout"])),
            nn.Linear(primary_evaluator_model_config["token_pred_dim_ff"],
                      primary_evaluator_model_config["token_pred_dim_ff"]),
            nn.GroupNorm(chunk_encoder_config["frame_enc_groups"], primary_evaluator_model_config["token_pred_dim_ff"]),
            nn.SELU(),
            nn.Dropout(float(chunk_encoder_config["dropout"])),
    ...
    def rate_alignment_batch(self, texts, motion_chunks, motion_masks, device):
        ...
        cls_embs = res_embs[:, 0, ...]
        text_acum_embs = torch.max(
            (res_embs * attn_masks[..., None])[:, 2:self.max_context_len - motion_embs.shape[1] - 1, :], dim=1).values
        motion_acum_embs = torch.max(
            (res_embs * attn_masks[..., None])[:, self.max_context_len - motion_embs.shape[1]:, :], dim=1).values
    ...
    def get_prob_and_cls_prediction(
        ...
        valid_text_embs = torch.max(
            (valid_res_embs * valid_attn_masks[..., None])[:, 2:self.max_context_len - motion_embs.shape[1] - 1, :],
            dim=1).values
        valid_motion_embs = torch.max(
            (valid_res_embs * valid_attn_masks[..., None])[:, self.max_context_len - motion_embs.shape[1]:, :],
            dim=1).values
        valid_text_sembs = torch.sum(
            (valid_res_embs * valid_attn_masks[..., None])[:, 2:self.max_context_len - motion_embs.shape[1] - 1, :],
            dim=1)
        valid_motion_sembs = torch.sum(
            (valid_res_embs * valid_attn_masks[..., None])[:, self.max_context_len - motion_embs.shape[1]:, :], dim=1)
