...
class Roberta(torch.nn.Module):
    ...
    def forward(self, text, device, **kwargs):
        # Set minimum length to 2 to prevent errors; prevents decoder from outputting NaN when there's only one token
...
class CLIP(torch.nn.Module):
    ...
    def forward(self, text, device, **kwargs):
        ...
        # print(x.shape, text_embed.shape)
        mask = lengths_to_mask(text.argmax(dim=-1), device)
        # print("ori mask:", mask)
        if mask.shape[1] < x.shape[1]:
            x = x[:, :mask.size(1)]
        # print(mask.size(1), x.shape)
