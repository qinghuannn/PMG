...
def mld_collate(batch):
    ...
    adapted_batch = {
        ...
        "keyframes": collate_tensors([b[7] for b in notnone_batches]),
        # "keyframes": torch.stack([b[7] for b in notnone_batches], dim=0),
        # "keyframes": torch.tensor(np.array([b[7] for b in notnone_batches]), dtype=torch.long)
    }
    if len(notnone_batches[0]) == 9:
        adapted_batch["pmg_motion"] = collate_tensors([torch.tensor(b[8]).float() for b in notnone_batches])
